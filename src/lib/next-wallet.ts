...
import { Network, WalletBehaviourFactory, BrowserWallet, Optional, Transaction, WalletModuleFactory } from "@near-wallet-selector/core";
import { createAction } from "@near-wallet-selector/wallet-utils";
import icon from "./icon";

export interface NextWalletParams {
  walletUrl?: string;
  iconUrl?: string;
  deprecated?: boolean;
}

interface NextWalletState {
  wallet: WalletConnection;
  keyStore: keyStores.BrowserLocalStorageKeyStore;
}

interface NextWalletExtraOptions {
  walletUrl: string;
}
...
const resolveWalletUrl = (network: Network, walletUrl?: string) => {
  ...
  switch (network.networkId) {
    case "mainnet":
      return "https://thenextwallet.app";
    case "testnet":
      return "https://testnet.thenextwallet.app";
    default:
...
const setupWalletState = async (
  params: NextWalletExtraOptions,
  network: Network
): Promise<NextWalletState> => {
...
const NextWallet: WalletBehaviourFactory<
  BrowserWallet,
  { params: NextWalletExtraOptions }
...
export function setupNextWallet({
  walletUrl,
  iconUrl = icon,
  deprecated = false,
}: NextWalletParams = {}): WalletModuleFactory<BrowserWallet> {
  return async () => {
    return {
      id: "next-wallet",
      type: "browser",
